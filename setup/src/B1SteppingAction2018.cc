#include "B1SteppingAction2018.hh"
...
B1SteppingAction2018::B1SteppingAction2018(B1EventAction* eventAction, B1RunAction* runAction, G4bool StoreCaloEnDepFlag, G4bool StoreGammaConvDepFlag, G4double EThr, const std::map<G4int,G4int> & ChannelMap, G4bool DetEnterExitFlag, const std::vector<G4int>  & TriggerLogic, G4bool Aug2018Flag)
...
B1SteppingAction2018::~B1SteppingAction2018()
{
  delete m_siDigitizer;
}

void B1SteppingAction2018::UserSteppingAction(const G4Step* step){
	 ...
	 // ########################
	       ...
	       if (NextVol && ThisVol->GetName()==worldName && ( NextVol->GetLogicalVolume()==fScoringVolume_PbG) ) {
		 ...
		 (runStepAction->GetVectorPbGlass2ExitPZ()).push_back(step->GetPostStepPoint()->GetMomentum().z()/GeV);
	       ...
	       // ########################
	       // What exits PbGlassG
	       if (NextVol && NextVol->GetName()==worldName && ( ThisVol->GetLogicalVolume()==fScoringVolume_PbG ) ) {
       ...
       if (fStoreGammaConvDepFlag && NextVol) { //Like "subdet", but for PostStep volume
	       ...
	       else if (Postvolume==fScoringVolume_T2)   {Postsubdet=11; }  //
	       else if (Postvolume==fScoringVolume_T3)   {Postsubdet=12; }  //
	       else if (Postvolume==fScoringVolume_Targ)   {Postsubdet=25; }  //
	       else if (Postvolume==fScoringVolume_T4) {Postsubdet=20; }  //
	       else if (Postvolume==fScoringVolume_T5)   {Postsubdet=21; }  //
	       else if (Postvolume==fScoringVolume_T6)   {Postsubdet=22; }  //
